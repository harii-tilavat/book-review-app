...
class BookService {
    ...
    async createBook(userId, book) {
        try {
            const { title, author, genreId, file, isbn, description, draftId = null } = book;
            if (!file) {
            ...
            const upadatedBook = { title, author, genreId, cover: uploadedFile.secure_url, description, isbn, draftId };
